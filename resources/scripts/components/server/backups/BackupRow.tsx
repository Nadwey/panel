...
import { IconArchive, IconDots, IconLock } from '@tabler/icons-react';
...
export default ({ backup, className }: Props) => {
    ...
    return (
        <GreyRowBox css={tw`flex-wrap md:flex-nowrap items-center overflow-visible`} className={className}>
            <div css={tw`flex items-center truncate w-full md:flex-1`}>
                <div css={tw`mr-4`}>
                    {backup.completedAt !== null ? (
                        backup.isLocked ? (
                            <IconLock css={tw`text-yellow-500`} />
                        ) : (
                            <IconArchive css={tw`text-neutral-300`} />
                        )
                    ) : (
                        <Spinner size={'small'} />
                    )}
